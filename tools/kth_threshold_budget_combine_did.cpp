...
bool cmpDidTPair(pair<int, short>& a, pair<int, short>& b){
...
unordered_map<string, pair<int64_t, int64_t>> load_lexicon(string& &gram_path, string &lexicon_path) {
    unordered_map<string, pair<int64_t, int64_t>> lex;

    ifstream in_gram(gram_path);
    ifstream in_lex(lexicon_path);
    string gramline, lexline;
...
double getTopKFromMap(unordered_map<string, unordered_map<int, short>> &t_did_map, int k, int considered_term, vector<uint32_t> &terms) {
    ...
    unordered_map<int, short> TopK;
    vector<pair<int, short>> TopKVec;
    ...
    for (size_t i = 0; i < terms.size(); ++i) {
        ...
        if (t_did_map.find(cur_term_string) != t_did_map.end()) {
            ...
            for (auto did_score : t_did_map[cur_term_string]) {
                if (TopK.find(did_score.first) == TopK.end()) {
                    TopK[did_score.first] = did_score.second;
                } else {
                    TopK[did_score.first] = max(TopK[did_score.first], did_score.second);
                }
            }
        }
    }
